...
1. `cloudflared`: <https://github.com/cloudflare/cloudflared/releases> altından `cloudflared-windows-amd64.exe` indirilir, SHA256 sürüm notundaki değerle karşılaştırılır (kurulum yolu: `%LOCALAPPDATA%\Programs\cloudflared\cloudflared.exe`).
2. Backend çalışırken: `cloudflared tunnel --url http://localhost:3001 --no-autoupdate`. Çıktıda `https://<rastgele>.trycloudflare.com` görünür. (`api.cloudflare.com` uyarısı yalnız otomatik güncellemeyi etkiler, tüneli değil.)
3. Iyzico sandbox panelinde (sandbox-merchant.iyzipay.com) bildirim adresi olarak `https://<rastgele>.trycloudflare.com/api/v1/payments/webhook` girilir. **Tünel her yeniden başlayınca adres değişir, panel de güncellenmelidir.**
4. Doğrulama: imzasız istek `403`, geçerli imzalı istek `200` dönmelidir (imza biçimi: [API.md § 7](API.md)). Gerçek bildirim gelince backend logunda `POST /api/v1/payments/webhook` satırı görünür.

**Bulgu (2026-09-26): sandbox hesabı V3 imzasını göndermiyor.** Gerçek bir ödemeden sonra Iyzico webhook'u geldi ama `X-IYZ-SIGNATURE-V3` yerine, kullanımdan kalkmış `X-Iyz-Signature` başlığı **boş değerle** geldi; imza doğrulaması bu yüzden `403` verdi. Iyzico dokümanına göre yalnız V3 desteklenir ve bunun için hesapta webhook imzası özelliğinin Iyzico entegrasyon ekibince açılması gerekir (<https://docs.iyzico.com/en/advanced/webhook>). Bakiye yine de doğru yüklenir: callback ve dakikalık mutabakat sonucu Iyzico'dan sorar, webhook yalnız hızlandırıcıdır. Canlıya geçmeden önce sandbox ve canlı hesap için V3 imzasının açılması Iyzico'dan talep edilmelidir. İmzasız webhook bilerek kabul edilmez. Reddedilen isteklerde backend hangi `x-iyz*` başlıklarının geldiğini ve gövde alanlarını (değerler değil) loglar. Iyzico başarısız bildirimi 15 dakikada bir, en fazla 3 kez tekrarlar.
