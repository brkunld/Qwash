...
- [ ] Webhook icin herkese acik adres: sandbox tuneli hazir ve imza dogrulamasi tunel uzerinden denendi (2026-09-26, `docs/DEVELOPMENT.md`); Iyzico paneline URL girildi, gercek bildirim geldi ama sandbox hesabi V3 imzasini gondermedi (bos `X-Iyz-Signature`), bu yuzden 403; Iyzico entegrasyon ekibinden hesap icin webhook imzasi (V3) acilmasi istenmeli, canli icin de sart. Bakiye callback + mutabakatla dogru yukleniyor. Canlida sabit alan adi.
...
- [x] Teknik hata iadesi (`SERVICE_FAILURE`): admin bir seans icin iade olusturur. `POST /admin/sessions/:id/service-refund` (ADMIN + SUPER_ADMIN; seans basina bir kez, ledger anahtari `service-refund:<seansId>`; tutar <= tahsil edilen; kaynak `SERVICE_REFUND`; denetim kaydi). Panel: inceleme kartlari ve `/sessions/:id` (kullanici hareketlerindeki "seans" baglantisi). **Karar (Burak, 2026-09-26): bakiyeye doner** (ledger'a ters kayit, karta gitmez). Gerekce: musteri peronda bekliyor, mikro tutarda Iyzico kismi iadesi ve FIFO karmasik; para sistem disina cikmadigi icin "bakiye iadesi yok" politikasinin amaci korunur. Vazgecen musteri destek/hesap kapatma akisiyla kart iadesi ister. Not: hesap silmedeki FIFO dagiliminda bu kredi kaynaksiz (IBAN) sayilir.
...
| 4 | Iade / para cikarma politikasi | **Kapandi (2026-09-26, Burak):** Bakiye iadesi yok. Iki istisna: (a) teknik hata / hizmet alamama (su, elektrik, vana, sistem arizasi) icin musteri destek talebi acar; (b) hesap silme / kullanimdan vazgecme durumunda kalan bakiye talep edilebilir. Iade yalniz paranin yuklendigi orijinal kart islemine Iyzico refund/cancel ile yapilir. Iade kullanici arayuzunden tek tikla yapilmaz: destek kanaliyla gelir, admin panelinden (Faz 6) yonetici onayiyla yapilir. **Ek (2026-09-26, Burak):** (1) Iade edilebilirlik FIFO: harcama en eski yuklemeden dusulmus sayilir, kalan bakiye en yeni yuklemelere aittir. (2) Iyzico iadesi odemeden sonra en fazla 365 gun mumkun; bu sureyi asan kisim yalniz musterinin kendi adina kayitli IBAN'a EFT ile (ad-soyad uyusmasi sart, aciklama `QWASH-REFUND-<USER_ID>`), admin onayiyla. (3) Hesap silmede uc secenek: bakiyeyi kullan (silme iptal), bakiyeden feragat (onay kutusu, ledger'a feragat kaydi, bakiye sifir), iade talebi. (4) 9-10 aydir hareketsiz bakiyeli hesaplara hatirlatma. **Nihai (2026-09-26, Burak):** (a) Ad ilk basarili kart yuklemesinde muhurlenir (`nameLockedAt`); sonra yalniz destek + kimlik beyaniyla degisir. IBAN iadesi otomatik degildir: admin EFT oncesi bankanin gosterdigi alici adini muhurlu adla gozle karsilastirir, uyusmazsa iptal. (b) Hesap silme = KVKK m.7 anonimlestirme: status DELETED, e-posta `deleted_<id>@anonymized.local`, ad "Silinmis Kullanici", telefon null, sifre/Google baglari ve oturumlar silinir; Wallet/Ledger silinmez (TTK m.82, VUK m.253, 10 yil). Iade talebi anonimlestirmeden once muhurlu adi, IBAN'i ve FIFO dagilimini kendi kaydina kopyalar (Jarvis). (c) Hatirlatma yalniz bilgilendirme dilinde (ETK Yon. m.6/2 transaksiyonel ileti, IYS onayi gerekmez): "Qwash hesabinizda X TL kullanilabilir bakiyeniz bulunmaktadir. Bu e-posta hesap durum bilgilendirmesi amaciyla gonderilmistir." (d) Nakit kaynakli kalan: istasyonda admin `CASH_REFUND` kaydiyla kasadan, gelemiyorsa muhurlu adli kendi IBAN'ina. |
